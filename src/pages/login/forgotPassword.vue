<template>
	<view class="forget-Password-wrapper loginsbg " > 
		...
		<view class="password-title">{{title}}</view>
		<login-input
	...
	export default {
		...
		data() {
			return {
				loginPhone: '',
				formConfig: [
					{label: '+86', placeholder: '请输入账号', key: 'loginPhone', type: 'number', maxlength: 11, clearable: true},
				],
				title:'忘记密码',
				isOnly:false
			}
		...
		onLoad(e) {
			this.title = e.title,
			this.isOnly = e.isOnly
			this.loginPhone = e.phone;
		},
		methods: {
			...
			next() {
				...
				if(checkFormToast(checkFormRules.isMobile(this.loginPhone, '请输入11位的有效手机号'))) {
					this.$navGoto('/pages/login/authCode?type=1&loginPhone=' + this.loginPhone)
				}
			},
		}
	}
